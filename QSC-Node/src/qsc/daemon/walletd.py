...
from qsc.daemon.helper import logger
from qsc.daemon.helper.DaemonHelper import WalletDecryptionError, Wallet, UNRESERVED_OTS_INDEX_START
from qsc.services.WalletAPIService import WalletAPIService
from qsc.generated import qsc_pb2, qsc_pb2_grpc, qscwallet_pb2
from qsc.generated.qscwallet_pb2_grpc import add_WalletAPIServicer_to_server
...
pid = os.path.join(config.user.wallet_dir, 'qsc_walletd.pid')


class WalletD:
    def __init__(self):
        self._wallet_path = os.path.join(config.user.wallet_dir, 'walletd.json')
        self._public_stub = qsc_pb2_grpc.PublicAPIStub(grpc.insecure_channel(config.user.public_api_server))
        self._wallet = None
        self._passphrase = None
        self.load_wallet()

    def to_plain_blocks(self, block):
        pheader = qscwallet_pb2.PlainBlockHeader()
        ...
        pblock = qscwallet_pb2.PlainBlock()
        ...
        for genesis_balance in block.genesis_balance:
            pgb = qscwallet_pb2.PlainGenesisBalance()
    ...
    def get_address_state(self, qaddress: str) -> AddressState:
        request = qsc_pb2.GetAddressStateReq(address=bytes(hstr2bin(qaddress[1:])))
    ...
    def is_slave(self, master_address: bytes, slave_pk: bytes) -> bool:
        request = qsc_pb2.IsSlaveReq(master_address=master_address,
                                     slave_pk=slave_pk)
    ...
    def get_unused_ots_index(self, address: bytes, unused_ots_index_from: int) -> Optional[int]:
        request = qsc_pb2.GetOTSReq(address=address,
    ...
    def relay_token_txn(self,
        ...
        for idx, qaddress in enumerate(qaddresses):
            initial_balances.append(qsc_pb2.AddressAmount(address=self.qaddress_to_address(qaddress),
                                                          amount=amounts[idx]))
    ...
    def get_mini_transactions_by_address(self, qaddress: str, item_per_page: int, page_number: int) -> tuple:
        ...
        response = self._public_stub.GetMiniTransactionsByAddress(
            qsc_pb2.GetMiniTransactionsByAddressReq(address=address,
    ...
    def get_transaction(self, tx_hash: str):
        ...
        response = self._public_stub.GetTransaction(qsc_pb2.GetTransactionReq(tx_hash=txhash))
    ...
    def get_balance(self, qaddress: str) -> int:
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetBalance(qsc_pb2.GetBalanceReq(address=address))
    ...
    def get_total_balance(self) -> int:
        ...
        response = self._public_stub.GetTotalBalance(qsc_pb2.GetTotalBalanceReq(addresses=addresses))
        return response.balance

    def get_ots(self, qaddress: str):
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetOTS(qsc_pb2.GetOTSReq(address=address))
        return response.ots_bitfield_by_page, response.next_unused_ots_index, response.unused_ots_index_found

    def get_height(self) -> int:
        response = self._public_stub.GetHeight(qsc_pb2.GetHeightReq())
        return response.height

    def get_block(self, header_hash: str):
        headerhash = bytes(hstr2bin(header_hash))
        response = self._public_stub.GetBlock(qsc_pb2.GetBlockReq(header_hash=headerhash))
        return self.to_plain_blocks(response.block)

    def get_block_by_number(self, block_number: int):
        response = self._public_stub.GetBlockByNumber(qsc_pb2.GetBlockByNumberReq(block_number=block_number))
        return self.to_plain_blocks(response.block)
    ...
    def get_node_info(self):
        return self._public_stub.GetNodeState(qsc_pb2.GetNodeStateReq())
...
def main():
    ...
    daemon = Daemonize(app="qsc_walletd", pid=pid, action=run)
    daemon.start()
