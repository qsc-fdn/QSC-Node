...
from qsc.core.Wallet import Wallet, WalletDecryptionError
from qsc.core.misc.helper import parse_hexblob, parse_qaddress
...
from qsc.crypto.xmss import XMSS, hash_functions
from qsc.generated import qsc_pb2_grpc, qsc_pb2

ENV_qsc_WALLET_DIR = 'ENV_qsc_WALLET_DIR'
...
class CLIContext(object):
    ...
    def get_stub_public_api(self):
        ...
        return qsc_pb2_grpc.PublicAPIStub(channel)
...
def _public_get_address_balance(ctx, address):
    ...
    get_address_state_req = qsc_pb2.GetAddressStateReq(address=parse_qaddress(address))
...
@click.option('--wallet_dir', default='.', help='local wallet dir', envvar=ENV_qsc_WALLET_DIR)
...
@qsc.command(name='wallet_recover')
...
@qsc.command(name='wallet_secret')
...
@qsc.command(name='wallet_encrypt')
@click.pass_context
...
@qsc.command(name='wallet_decrypt')
@click.pass_context
...
def tx_push(ctx, txblob):
    ...
    stub = ctx.obj.get_stub_public_api()
    pushTransactionReq = qsc_pb2.PushTransactionReq(transaction_signed=tx.pbdata)
...
@qsc.command(name='tx_multi_sig_create')
...
@qsc.command(name='tx_multi_sig_spend')
@click.option('--src', type=str, default='', prompt=True, help='signer QRL address')
...
def tx_token(ctx, src, master, symbol, name, owner, decimals, fee, ots_key_index):
    ...
    while True:
        ...
        initial_balances.append(qsc_pb2.AddressAmount(address=parse_qaddress(address),
                                                      amount=amount))
...
@qsc.command(name='tx_transfertoken')
@click.option('--src', type=str, default='', prompt=True, help='source QRL address')
@click.option('--master', type=str, default='', prompt=True, help='master QRL address')
...
@qsc.command(name='slave_tx_generate')
...
def token_list(ctx, owner):
    ...
    try:
        stub = ctx.obj.get_stub_public_api()
        address_state_req = qsc_pb2.GetAddressStateReq(address=owner_address)
        ...
        for token_hash in address_state_resp.state.tokens:
            get_object_req = qsc_pb2.GetObjectReq(query=bytes(hstr2bin(token_hash)))
...
def state(ctx):
    ...
    """
    stub = ctx.obj.get_stub_public_api()
    nodeStateResp = stub.GetNodeState(qsc_pb2.GetNodeStateReq())
