...
from qsc.core import config, BlockHeader
from qsc.core.config import DevConfig
from qsc.core.formulas import block_reward
from qsc.core.StateContainer import StateContainer
from qsc.core.StateMigration import StateMigration
from qsc.core.AddressState import AddressState
from qsc.core.VoteStats import VoteStats
from qsc.core.LastTransactions import LastTransactions
from qsc.core.TransactionMetadata import TransactionMetadata
from qsc.core.OptimizedAddressState import OptimizedAddressState
from qsc.core.PaginatedData import PaginatedData
from qsc.core.PaginatedBitfield import PaginatedBitfield
from qsc.core.Indexer import Indexer
from qsc.core.Block import Block
from qsc.core.BlockMetadata import BlockMetadata
from qsc.core.DifficultyTracker import DifficultyTracker
from qsc.core.GenesisBlock import GenesisBlock
from qsc.core.MultiSigAddressState import MultiSigAddressState
from qsc.core.PoWValidator import PoWValidator
...
from qsc.core.TransactionPool import TransactionPool
from qsc.core.misc import logger
from qsc.crypto.Qryptonight import Qryptonight
from qsc.generated import qsc_pb2, qscstateinfo_pb2
...
class ChainManager:
    ...
    def get_slave_pk_access_type(self, address: bytes, slave_pk: bytes) -> qsc_pb2.SlaveMetadata:
        with self.lock:
    ...
    def get_token(self, address: bytes, token_txhash: bytes) -> qsc_pb2.TokenBalance:
        with self.lock:
    ...
    def get_headerhashes(self, start_blocknumber):
        with self.lock:
            ...
            node_header_hash = qsc_pb2.NodeHeaderHash()
    ...
    def load(self, genesis_block):
        ...
        if height == -1:
            ...
            block_number_mapping = qsc_pb2.BlockNumberMapping(headerhash=genesis_block.headerhash,
    ...
    def _try_branch_add_block(self, block, dev_config: DevConfig, check_stale=True) -> bool:
        ...
        if new_block_difficulty > last_block_difficulty:
            if self._last_block.headerhash != block.prev_headerhash:
                fork_state = qscstateinfo_pb2.ForkState(initiator_headerhash=block.headerhash)
    ...
    def _rollback(self, forked_header_hash: bytes, fork_state: qscstateinfo_pb2.ForkState = None):
        """
    ...
    def add_chain(self, hash_path: list, fork_state: qscstateinfo_pb2.ForkState) -> bool:
        """
    ...
    def _fork_recovery(self, block: Block, fork_state: qscstateinfo_pb2.ForkState) -> bool:
    ...
    def _update_block_number_mapping(self, block, batch):
        block_number_mapping = qsc_pb2.BlockNumberMapping(headerhash=block.headerhash,
    ...
    def update_state_container(self,
        ...
        if isinstance(tx, TransferTokenTransaction):
            ...
            if key not in state_container.lattice_pk.data:
                lattice_pk.load(key, qsc_pb2.LatticePKMetadata())
    ...
    def get_block_datapoint(self, headerhash):
        ...
        data_point = qsc_pb2.BlockDataPoint()
