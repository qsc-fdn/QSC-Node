...
from qsc.core.txs import build_tx as main_build_tx
from qsc.crypto.misc import sha256
...
class Transaction(object, metaclass=ABCMeta):
    ...
    def __init__(self, protobuf_transaction=None):
        ...
        if protobuf_transaction is None:
            self._data = qsc_pb2.Transaction()
    ...
    @staticmethod
    def from_pbdata(pbdata: qsc_pb2.Transaction):
    ...
    def validate_or_raise(self, verify_signature=True) -> bool:
        ...
        if verify_signature:
            ...
            # Note: revised by Jim
            #print(f'-> validate signature with XMSS: ')
            #print(f'--> data={self.get_data_hash()}')
            #print(f'--> signature={self.signature}')
            #print(f'--> PK={self.PK}')
            if not XMSS.validate_signature(self.get_data_hash(), self.signature, self.PK):
                raise ValueError("Invalid xmss signature")

            #if not XmssFast.verify(self.get_data_hash(), self.signature, self.PK):
            #    raise ValueError("Invalid xmss signature")
