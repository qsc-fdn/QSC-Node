...
from qsc import __version__ as version
...
class UserConfig(object):
    ...
    def __init__(self, ignore_check=False):
        ...
        self.genesis_prev_headerhash = b'in the middle of every difficulty lies opportunity' # b'The sleeper must awaken'
        self.genesis_timestamp = 1739612054 # changed
        self.genesis_difficulty = 10000 # original: 10000000
        ...
        self.peer_list = ['10.6.64.193', '10.6.64.148'] #['101.78.229.4', '14.136.245.194'] # ???

        self.p2p_local_port = 19001  # Locally binded port at which node will listen for connection, changed from 19000 
        self.p2p_public_port = 19001  # Public port forwarding connections to server, changed from 19000
        ...
        self._qsc_dir = expanduser(os.path.join("~/.qsc")) # changed
    ...
    @property
    def qsc_dir(self):
        return self._qsc_dir

    @qsc_dir.setter
    def qsc_dir(self, new_qsc_dir):
        self._qsc_dir = new_qsc_dir
        self.load_yaml(self.config_path)

    @property
    def wallet_dir(self):
        return expanduser(self.qsc_dir)

    @property
    def data_dir(self):
        return expanduser(os.path.join(self.qsc_dir, "data"))

    @property
    def config_path(self):
        return expanduser(os.path.join(self.qsc_dir, "config.yml")) # TODO

    @property
    def log_path(self):
        return expanduser(os.path.join(self.qsc_dir, "qsc.log"))

    @property
    def walletd_log_path(self):
        return expanduser(os.path.join(self.qsc_dir, "walletd.log"))

    @property
    def mining_pool_payment_wallet_path(self):
        return expanduser(os.path.join(self.qsc_dir, 'payment_slaves.json'))
...
class DevConfig(object):
    ...
    def __init__(self, pbdata, ignore_check=False, ignore_singleton=False):
        ...
        self.version = '0.1.1' #version + ' python'
        ...
        self.supplied_coins = 60000000 * (10 ** 9) # original: 65000000
        ...
        # ======================================
        self.hard_fork_heights = [1, 2, 3] # original: [942375, 1938000, 2078800] # [0, 0, 0] will crash generate_genesis.py?
        self.hard_fork_node_disconnect_delay = [0, 0, 0] # original: [0, 0, 2880] 
    ...
    def coin_remaining_at_genesis(self):
        return decimal.Decimal(self.pbdata.chain.max_coin_supply - 60000000) # original: 65000000
    ...
    def create(prev_state_key,
        ...
        chain = qsc_pb2.DevConfig.Chain(reorg_limit=300,
                                        max_coin_supply=100000000, # orginal: 105000000
                                        complete_emission_time_span_in_years=200)
        block_size_controller = qsc_pb2.DevConfig.Block.BlockSizeController(number_of_blocks_analyze=10,
                                                                            size_multiplier=110,  # 1.1
                                                                            block_min_size_limit_in_bytes=1024 * 1024)
        block = qsc_pb2.DevConfig.Block(mining_nonce_offset=39,
                                        extra_nonce_offset=43,
                                        mining_blob_size_in_bytes=76,
                                        block_timing_in_seconds=60,
                                        block_size_controller=block_size_controller)

        message = qsc_pb2.DevConfig.Transaction.Message(max_length=80)

        slave = qsc_pb2.DevConfig.Transaction.Slave(slave_pk_max_length=67)

        token = qsc_pb2.DevConfig.Transaction.Token(symbol_max_length=10,
                                                    name_max_length=30)

        lattice = qsc_pb2.DevConfig.Transaction.Lattice(pk1_max_length=1088,
                                                        pk2_max_length=1472,
                                                        pk3_max_length=65)

        foundation_multi_sig = qsc_pb2.DevConfig.Transaction.FoundationMultiSig(threshold_percentage=10)

        # proposal_config_activation_delay should not be less than reorg_limit
        proposal = qsc_pb2.DevConfig.Transaction.Proposal(threshold_per=51,
        ...
        transaction = qsc_pb2.DevConfig.Transaction(multi_output_limit=100,
                                                    message=message,
                                                    slave=slave,
                                                    token=token,
                                                    lattice=lattice,
        ...
        dev_config = qsc_pb2.DevConfig(chain=chain,
                                       block=block,
                                       transaction=transaction,
                                       pow=qsc_pb2.DevConfig.POW(N_measurement=30, kp=5))
...
# Base64 string
base64_string = "ijHyscz4ePJ/5C+y6E3l2xqA+Zg8DBeko96d58s5ud0=" #"O4dhrAycCtlvr9uNXNx1Revj/lkx8A8g6jWB1GsDMas=" # NOTE: needs change if genesis.yml changed!
...
genesis_header_hash = bytes(hstr2bin(hex_string)) 
