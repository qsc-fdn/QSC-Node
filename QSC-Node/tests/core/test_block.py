...
from qsc.core.BlockHeader import BlockHeader
...
from qsc.crypto.misc import merkle_tx_hash
...
class TestBlock(TestCase):
    ...
    def test_validate(self):
        ...
        with patch('qsc.core.Block.Block.transactions', new_callable=PropertyMock) as m_transactions:
            m_transactions.return_value = []
            result = self.block.validate(m_chain_manager, OrderedDict())
            self.assertFalse(result)

        # There was a problem with the CoinBase TX
        with patch('qsc.core.txs.CoinBase.CoinBase._validate_extended') as m_validate_extended:
...
@patch('qsc.core.misc.ntp.getTime', new=replacement_getTime)
@patch('qsc.core.txs.CoinBase.CoinBase.apply', name='mock CoinBase.apply()',
       return_value=True)
@patch('qsc.core.txs.TransferTransaction.TransferTransaction.apply',
       name='mock TransferTransaction.apply()', return_value=True)
@patch('qsc.core.txs.Transaction.Transaction.validate_slave',
       name='mock Transaction.validate_slave()', return_value=True)
@patch('qsc.core.txs.TransferTransaction.TransferTransaction.validate', name='mock Transfer.validate()',
       return_value=True)
class TestBlockApplyStateChanges(TestCase):
    ...
    def test_last_block(self,
        ...
        with patch("qsc.core.Block.Block.get_block_by_number") as mock_get_block_by_number:
