...
class TestP2PPeerManager(TestCase):
    ...
    @patch('qsc.core.p2p.p2pPeerManager.logger', autospec=True)
    ...
    @patch('qsc.core.p2p.p2pPeerManager.logger', autospec=True)
    @patch('qsc.core.p2p.p2pPeerManager.config', autospec=True)
    ...
    @patch('qsc.core.p2p.p2pprotocol.P2PProtocol.peer')
    @patch('qsc.core.p2p.p2pprotocol.P2PProtocol.send')
    def test_trusted_message_count(self, send, get_peer):
        channel = P2PProtocol()
        with mock.patch('qsc.core.misc.ntp.getTime') as time_mock:
