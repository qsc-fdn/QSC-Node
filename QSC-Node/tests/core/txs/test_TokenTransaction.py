...
@patch('qsc.core.txs.Transaction.logger')
class TestTokenTransaction(TestCase):
    ...
    @patch('qsc.core.txs.Transaction.Transaction.validate_slave', return_value=True)
    def test_validate_extended(self, m_validate_slave, m_logger):
        ...
        with patch('qsc.core.txs.TokenTransaction.TokenTransaction.addr_from',
                   new_callable=PropertyMock) as m_addr_from:
            m_addr_from.return_value = b'Invalid Address'
            result = tx._validate_extended(state_container)
            self.assertFalse(result)

        with patch('qsc.core.txs.TokenTransaction.TokenTransaction.owner', new_callable=PropertyMock) as m_owner:
            m_owner.return_value = b'Invalid Address'
            result = tx._validate_extended(state_container)
            self.assertFalse(result)

        with patch('qsc.core.txs.TokenTransaction.TokenTransaction.initial_balances',
        ...
        # Fee is -1
        with patch('qsc.core.txs.TokenTransaction.TokenTransaction.fee', new_callable=PropertyMock) as m_fee:
        ...
        # tx = self.make_tx()
        # tx.sign(self.alice)
        # with patch('qsc.core.txs.TokenTransaction.TokenTransaction.initial_balances', new_callable=PropertyMock) as m_i_balances:
